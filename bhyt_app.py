...
with tab1:
    ...
    with st.sidebar:
        ...
        uploaded_icd = st.file_uploader("1. ICD10.xlsx (cột B = mã ICD-10) - bắt buộc", type=["xlsx"], key="icd_tab1")
        if uploaded_icd:
    ...
    if st.button("🚀 BẮT ĐẦU KIỂM TRA TẤT CẢ", type="primary", use_container_width=True):
        if not os.path.exists("ICD10.xlsx"):
            st.error("❌ Vui lòng tải file ICD10.xlsx ở thanh bên (sidebar) trước khi kiểm tra!")
        else:
            VALID_MA_BENH_CODES = load_icd10_codes()
